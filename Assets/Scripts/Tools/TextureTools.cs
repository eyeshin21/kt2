...
public class ResizeTexture2D : Utilities
{
    ...
    void Review()
    {
        ...
        if (selectedObjects != null)
        {
            ...
            else if (optionsChoose == Options.Resize)
            {
    ...
    void ChooseOptions(Options options)
    {
        switch (options)
        {
            ...
            case Options.Crop:
                ...
                break;
            case Options.AutoResizePOT:
                Review();
                if (GUILayout.Button("Auto Resize POT"))
                {
                    AutoResizeTexturePOT();
                }
                break;
        }
    ...
    private void AutoResizeTexturePOT()
    {
        Debug.Log("auto resize POT");
        Debug.Log(listTexture2D.Count);
        for (int index = 0; index < listTexture2D.Count; index++)
        {
            int tempWidth = listTexture2D[index].width;
            if (tempWidth % 4 == 1)
            {
                tempWidth -= 1;
            }
            else if (tempWidth % 4 == 2)
            {
                tempWidth += 2;
            }
            else if (tempWidth % 4 == 3)
            {
                tempWidth += 1;
            }
            int tempHeight = listTexture2D[index].height;
            if (tempHeight % 4 == 1)
            {
                tempHeight -= 1;
            }
            else if (tempHeight % 4 == 2)
            {
                tempHeight += 2;
            }
            else if (tempHeight % 4 == 3)
            {
                tempHeight += 1;
            }
            TextureScale.Bilinear(listTexture2D[index], tempWidth, tempHeight);
